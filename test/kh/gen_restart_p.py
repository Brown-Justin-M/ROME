...
z = np.array(d["T"][:]) * 0.0 + z[:,np.newaxis,np.newaxis] - np.mean(z)

d["T"][:] = params["strat_t"] * grid["z_length"] * np.tanh((z + 0.1 * np.sin(2.0 * np.pi * x / grid["x_length"])/0.5)) / 2.0 - params["strat_t"] * z
d["C"][:] = 0.0
d["U"][:] = grid["x_shear"] * grid["z_length"] * np.tanh((z + 0.1 * np.sin(2.0 * np.pi * x / grid["x_length"]))/0.5) / 2.0 - grid["x_shear"] * z
d["V"][:] = 0.0
d["W"][:] = 0.0
